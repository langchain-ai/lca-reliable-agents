"""
Agent v5 adapted for evaluation / batch running.

Changes from agent_v5.py:
- No conversation history — each chat() call is isolated
- Fixed db_path to inventory/inventory_with_prices.db
- Knowledge base loads on import
- chat() accepts a string and returns a string (the agent's response)
"""
...
# Configuration
DB_PATH = "inventory/inventory_with_prices.db"
...
# ---------------------------------------------------------------------------
# System prompt (unchanged from agent_v5)
# ---------------------------------------------------------------------------
...
@traceable(run_type="tool", name="query_database")
...
async def load_knowledge_base(kb_dir: str = "knowledge_base") -> None:
    """Load knowledge base documents and generate embeddings for WHOLE documents."""
...
@traceable(run_type="tool", name="search_knowledge_base")
...
# ---------------------------------------------------------------------------
# Chat — isolated, no history
# ---------------------------------------------------------------------------

@traceable(name="Emma")
async def chat(question: str) -> str:
    """Process a single question and return the agent's response. No history."""
    ...
    while response_message.tool_calls:
        ...
        for tool_call in response_message.tool_calls:
            ...
            if tool_call.function.name == "query_database":
                result = query_database(query=function_args.get("query"))
            elif tool_call.function.name == "search_knowledge_base":
                result = await search_knowledge_base(query=function_args.get("query"))
            else:
...
async def ensure_kb_loaded():
    """Load the knowledge base if not already loaded. Call this before chat()."""
    global _kb_loaded
    if not _kb_loaded:
