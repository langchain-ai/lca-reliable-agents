...
let knowledgeBaseDocs: [string, string][] = []; // List of [chunk_name, content] tuples
...
const searchKnowledgeBase = traceable(
  async (query: string, topK: number = 2): Promise<string> => {
    ...
    for (const [idx, score] of topResults) {
      const [chunkName, content] = knowledgeBaseDocs[idx];
      results.push(`=== ${chunkName} (relevance: ${score.toFixed(3)}) ===\n${content}\n`);
    }
