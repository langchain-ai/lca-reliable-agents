...
const threadStore: Map<string, any[]> = new Map();
...
let knowledgeBaseDocs: [string, string][] = [];
let knowledgeBaseEmbeddings: number[][] = [];
...
async function loadKnowledgeBase(
  ...
  if (fs.existsSync(cachePath)) {
    ...
    knowledgeBaseDocs = cacheData.docs.map(
      (doc: [string, string]) => [doc[0], doc[1]] as [string, string]
    );
    knowledgeBaseEmbeddings = cacheData.embeddings;
    console.log(
      `Knowledge base loaded from cache: ${knowledgeBaseDocs.length} chunks`
    );
    return;
  }
...
const QUERY_DATABASE_TOOL: OpenAI.ChatCompletionTool = {
  type: "function",
  function: {
    name: "query_database",
    description:
...
const SEARCH_KNOWLEDGE_BASE_TOOL: OpenAI.ChatCompletionTool = {
  type: "function",
  function: {
    name: "search_knowledge_base",
    description:
      "Search company knowledge base for information about policies, procedures, company info, shipping, returns, ordering, contact information, store locations, and business hours. Use this for non-product questions.",
    parameters: {
      type: "object",
      properties: {
        query: {
  ...
  while (responseMessage.tool_calls) {
    ...
    for (const toolCall of responseMessage.tool_calls) {
      ...
      if (toolCall.function.name === "query_database") {
        result = queryDatabase(functionArgs.query, dbPath);
