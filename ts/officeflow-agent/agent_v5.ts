...
async function generateAndCacheEmbeddings(kbPath: string, cachePath: string): Promise<void> {
  ...
  if (docs.length === 0) {
    console.log(`Warning: No documents found in '${kbPath}'`);
    return;
  }
...
async function loadKnowledgeBase(kbDir: string = "./knowledge_base"): Promise<void> {
  ...
  if (embeddingsAreStale(kbPath, cachePath)) {
    ...
    console.log(`Knowledge base loaded from cache: ${knowledgeBaseDocs.length} documents`);
  }
}
...
function setThreadId(id: string) { threadId = id; }

export { chat, loadKnowledgeBase, threadId, setThreadId };
